...
$querySubcategories = "SELECT * FROM tbl_gallery_subcategory WHERE status = 1";
...
$queryGallery = "SELECT * FROM tbl_gallery WHERE status = 1";
$resultGallery = mysqli_query($conn, $queryGallery);
$galleryData = mysqli_fetch_all($resultGallery, MYSQLI_ASSOC);
?>
<!doctype html>
...
<body class="  ">
    ...
    <main class="main-content">
                                    ...
                                    <div class="col-lg-4 col-md-4 col-sm-6 col-12">
                                        <div class="card h-100" style="padding: 20px 0;filter: drop-shadow(3px 4px 14px rgba(0,0,0,.15));">
                                            <a class='width-fit-content btn btn-sm btn-icon btn-danger delete-btn gallery-delete-btn'
                                                data-id="<?= $gallery['id']; ?>" data-bs-toggle='tooltip'
                                            ...
                                            <h6 class="card-title text-center mb-0">
                                                <?= $gallery['subcategory_name']; ?>
                                            </h6>                                    
                                            <img style="width: 170px;height:170px;background: #E6F5F6;padding:10px" src="uploads/<?= $gallery['image'] ?>"
                                                    class="img-fluid rounded m-auto" alt="profile-image">                                            
                                            <h6 class="text-center mb-2 mx-4">
                                                <?= $gallery['title']; ?>
                                            </h6>
        ...
        <!-- Footer Section End -->
        <!-- add gallery popup - start -->
        <div id="overlay">
            <div id="popup">
                <h5 class="text-center fw-600 text-primary text-uppercase mb-2">Add Gallery</h5>
                <form id="galleryForm">
                    <div class="row">
                        <!-- Category Dropdown -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#galleryForm").submit(function (e) {
                ...
                if (!categoryId || !subcategoryId || !imageInput || title.trim()==='' || description.trim()==='') {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'backend-gallery', // Replace with your backend script
                    data: formData,
                    contentType: false,
